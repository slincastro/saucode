...
export class ViewUtils {
   ...
   */
  public static formatMetricsAsChartHtml(originalMetrics: Metrics, improvedMetrics: Metrics): string {
    ...
    for (const metric of commonMetrics) {
      ...
      if (originalMetric) {
        (metricsData.before as any)[metric.key] = originalMetric.value;
      }

      if (improvedMetric) {
        (metricsData.after as any)[metric.key] = improvedMetric.value;
      }
    }
    ...
    if (Object.keys(metricsData.before).length === 0 || Object.keys(metricsData.after).length === 0) {
      ...
      metricArray.forEach((name, index) => {
        ...
        if (originalMetric) {
          (metricsData.before as any)[`metric_${index}`] = originalMetric.value;
        }

        if (improvedMetric) {
          (metricsData.after as any)[`metric_${index}`] = improvedMetric.value;
        }
      });
    }
